...
<?php
	$date = date("Y/m/d");
	$statement = $pdo->prepare("UPDATE tbl_donor SET status=? , don =? WHERE donor_id=?");
	$statement->execute(array(1,$date,$_REQUEST['id']));
	$statement = $pdo->prepare("UPDATE tbl_blood_group SET stock=stock+?  WHERE blood_group_id=?");
	$statement->execute(array(0.47,$blood_group_id));
