...
if (isset($_POST['form1'])) {
	...
	if(empty($_POST['agent_blood_group_id'])) {
        $valid = 0;
        $error_message .= "Blood Group can not be empty.\\n";
    }
    ...
    if($valid == 1) {
		...
		if(empty($_POST['agent_password'])) {
			$statement = $pdo->prepare("UPDATE tbl_agent SET agent_name=?, agent_designation=?, agent_organization=?, agent_description=?, agent_phone=?, agent_website=?, agent_address=?, agent_city=?, agent_state=?, agent_country=?, agent_zip_code=?, agent_map=?, agent_facebook=?, agent_twitter=?, agent_linkedin=?, agent_googleplus=?, agent_pinterest=?,agent_blood_group=? WHERE agent_id=?");
			$statement->execute(array($_POST['agent_name'],$_POST['agent_designation'],$_POST['agent_organization'],$_POST['agent_description'],$_POST['agent_phone'],$_POST['agent_website'],$_POST['agent_address'],$_POST['agent_city'],$_POST['agent_state'],$_POST['agent_country'],$_POST['agent_zip_code'],$_POST['agent_map'],$_POST['agent_facebook'],$_POST['agent_twitter'],$_POST['agent_linkedin'],$_POST['agent_googleplus'],$_POST['agent_pinterest'],$_POST['agent_blood_group_id'],$_SESSION['agent']['agent_id']));	
		} else {
			$statement = $pdo->prepare("UPDATE tbl_agent SET agent_name=?, agent_designation=?, agent_organization=?, agent_description=?, agent_phone=?, agent_website=?, agent_address=?, agent_city=?, agent_state=?, agent_country=?, agent_zip_code=?, agent_map=?, agent_facebook=?, agent_twitter=?, agent_linkedin=?, agent_googleplus=?, agent_pinterest=?,agent_blood_group=?,agent_password=? WHERE agent_id=?");
			$statement->execute(array($_POST['agent_name'],$_POST['agent_designation'],$_POST['agent_organization'],$_POST['agent_description'],$_POST['agent_phone'],$_POST['agent_website'],$_POST['agent_address'],$_POST['agent_city'],$_POST['agent_state'],$_POST['agent_country'],$_POST['agent_zip_code'],$_POST['agent_map'],$_POST['agent_facebook'],$_POST['agent_twitter'],$_POST['agent_linkedin'],$_POST['agent_googleplus'],$_POST['agent_pinterest'],$_POST['agent_blood_group_id'],md5($_POST['agent_password']),$_SESSION['agent']['agent_id']));
		...
		$_SESSION['agent']['agent_blood_group'] = $_POST['agent_blood_group_id'];
    }
}
?>
								...
								<div class="login-form">
									...
									<form action="" method="post">
										...
										<div class="form-row">
											...
											<select data-placeholder="Choose a blood group" class="form-control chosen-select" name="agent_blood_group_id">
												<?php
												...
												foreach ($result as $row) {
													?>
													<option></option>
													<option value="<?php echo $row['blood_group_id']; ?>" <?php if($row['blood_group_id'] == $_SESSION['agent']['agent_blood_group']) {echo 'selected';} ?>><?php echo $row['blood_group_name']; ?></option>
													<?php
												}
												?>
											</select>
